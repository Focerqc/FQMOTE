...
test("device rejection, send failure and cancellation reject and remove listeners", async () => {
  ...
  await assert.rejects(
    requestSettingsJson(failing, "settings", "settings"),
    /disconnected/,
  );
  assert.equal(failing.listeners.size, 0);
  const controller = new AbortController();
  const cancelled = requestSettingsJson(
    serial,
    "settings",
    "settings",
    controller.signal,
...
test("synchronous send failure removes listeners and timers", async () => {
  ...
  try {
    ...
    await assert.rejects(
      requestSettingsJson(serial, "settings", "settings"),
      /port closed/,
    );
    assert.equal(serial.listeners.size, 0);
...
test("numeric metadata validates brightness and colour boundaries", () => {
  ...
  for (const value of [10, 255])
    assert.equal(
      schema.safeParse({ ...metadata.values, brightness: value }).success,
      true,
    );
  ...
  if (metadata.fields[2].type !== "range")
    throw new Error("Expected range field");
  ...
  for (const value of [0, 0xffffff])
    assert.equal(
      colorSchema.safeParse({ ...metadata.values, brightness: value }).success,
      true,
    );
